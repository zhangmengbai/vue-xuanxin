import ratio from "common/js/MAIN"
import {SimplexNoise} from "common/js/lib/perlin-simplex"
import tombola from "common/js/lib/tombola"


export function Paint(ctx,width,height,scale,col1,col2,col3,contrast,banding) {
    ...
    this.wobbleHeight = tombola.prototype.rangeFloat(17,26) * scale;
...
Paint.prototype._newRow = function() {
    ...
    this.rowOffset += tombola.prototype.rangeFloat(-10,10);

    // progress perlin vertical index for color //进展perlin垂直指数的颜色
    var sm = 0.6;
    this.streakIndex += tombola.prototype.rangeFloat(-0.05 * sm,0.05 * sm);
    if (tombola.prototype.percent(1.2 * this.pScale)) {
        this.streakIndex += tombola.rangeFloat(0.2 * sm,0.3 * sm); // larger jump
    }
    else if (tombola.prototype.percent(0.7 * this.pScale)) {
